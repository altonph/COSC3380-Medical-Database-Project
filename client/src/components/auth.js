...
export function useRequireAuthAdmin(role = 'Admin') {
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    const checkTokenExpiry = async () => {
      try {
        const response = await fetch("https://cosc3380-medical-database-project-server.onrender.com/api/admin/protected", {
          method: "GET",
          headers: { "Authorization": `Bearer ${token}` }
        });
...
export function useRequireAuthDoctor(role = 'Dentist') {
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    const checkTokenExpiry = async () => {
      try {
        const response = await fetch("https://cosc3380-medical-database-project-server.onrender.com/api/doctor/protected", {
          method: "GET",
          headers: { "Authorization": `Bearer ${token}` }
        });
...
export function useRequireAuthStaff(role = 'Staff') {
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    const checkTokenExpiry = async () => {
      try {
        const response = await fetch("https://cosc3380-medical-database-project-server.onrender.com/api/staff/protected", {
          method: "GET",
          headers: { "Authorization": `Bearer ${token}` }
        });
