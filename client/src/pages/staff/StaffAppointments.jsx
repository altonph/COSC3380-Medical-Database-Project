...
const StaffAppointment = () => {
    ...
    const handleAddVisitDetails = async () => {
      try {
          ...
          window.location.href = "/staff/appointments/add-visit-details";
      } catch (error) {
    ...
    useEffect(() => {
      const fetchAppointments = async () => {
        try {
          const token = localStorage.getItem('token'); 
          const response = await fetch('https://cosc3380-medical-database-project-server.onrender.com/api/staff/appointments', {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
